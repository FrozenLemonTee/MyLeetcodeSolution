package isPalindrome;

public class test {
    public static void main(String[] args) {
        System.out.println(new Solution().isPalindrome(121));
        System.out.println(new Solution().isPalindrome(123));
        System.out.println(new Solution().isPalindrome(-121));
        System.out.println(new Solution().isPalindrome(12));
        System.out.println(new Solution().isPalindrome(0));
        System.out.println(new Solution().isPalindrome(1));
        System.out.println();
        System.out.println(new Solution().isPalindrome2(121));
        System.out.println(new Solution().isPalindrome2(123));
        System.out.println(new Solution().isPalindrome2(-121));
        System.out.println(new Solution().isPalindrome2(12));
        System.out.println(new Solution().isPalindrome2(0));
        System.out.println(new Solution().isPalindrome2(1));
        System.out.println();
        System.out.println(new Solution().isPalindrome3(121));
        System.out.println(new Solution().isPalindrome3(123));
        System.out.println(new Solution().isPalindrome3(-121));
        System.out.println(new Solution().isPalindrome3(12));
        System.out.println(new Solution().isPalindrome3(0));
        System.out.println(new Solution().isPalindrome3(1));
        System.out.println(new Solution().isPalindrome3(20));
    }
}
